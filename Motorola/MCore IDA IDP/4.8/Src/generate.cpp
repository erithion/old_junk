...
bool generate_c::outOperandMemory(const insn_t& ins, const op_t& x)
{
	...
	if (::hasRef(::getFlags(refEa)))
	{
		...
		const char* ptr = get_name_expr(refEa, cp.n, target, BADADDR);
...
bool generate_c::outOperandNear(const insn_t& ins, const op_t& x)
{
	...
	const char*           ptr = get_name_expr(ins.ea + cp.offb, cp.n, ea, cp.addr);
...
void generate_c::generateSegmentHeader(ea_t ea)
{
	...
	mcore_n::string      sname(get_true_segm_name(seg));
	mcore_n::string      sclass(get_segm_class(seg));
	bool                 sfound = false;
...
void generate_c::generateSegmentFooter(ea_t ea)
{
	...
	gen_cmt_line("%-*s ends", inf.indent - strlen(ash.cmnt) - 2, get_segm_name(seg));
}

...
//-------------------------------------------------------------------------
// Values of segment registers are kept is address ranges where a register
// doesn't change its value. The IDP module finds segment register change
// points and splits segreg_t areas so that a new segreg_t area is started
// at each segment register change point. The kernel deletes segreg_t
...
// range of addresses where segment registers don't change their values.
//
// A segment register is 16-bit quantity. A value BADSEL is reserved and
// represents an unknown value of a segment register.
//
// Note that each segment has information about the default values of
// segment registers. This information is used if value of a segment
// register is unknown.
//

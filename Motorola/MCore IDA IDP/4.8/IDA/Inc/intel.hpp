...
{
  return mode64()
      && ((cmd.rex & REX_W) != 0 || insn_default_opsize_64()); // 64-bit segment, rex.w or insns-64
}
...
const char *get_type_name(flags_t flag,ea_t ea_or_id);
...
int is_jump_func(func_t *pfn, ea_t *jump_target);
...
//------------------------------
typedef struct { // loaded from ida.int
  ...
  char   strings[1];  // name [param] [comment]
}vxd_info;

vxd_info *vxd_information(void);
...
bool should_ignore_micro(ea_t ea);
//-------------------------------------------------------------------------

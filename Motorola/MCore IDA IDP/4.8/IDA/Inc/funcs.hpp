...
// In fact this function with function chunks too
...
inline bool set_func_comment(func_t *fn, const char *cmt, bool repeatable)
                { return funcs.set_area_comment(fn, cmt, repeatable); }


// Delete function comment
//      fn        - ptr to function structure
//      repetable - delete repetable comment?
// In fact this function with function chunks
...
//inline bool idaapi is_32bit_func(func_t *pfn) { return get_func_bitness(pfn) >= 1; }
...
typedef bool idaapi testf_t(flags_t flags);     // flag tester
...
prefix bool ida_export func_item_iterator_next(func_item_iterator_t *fii, testf_t *testf);\
prefix bool ida_export func_item_iterator_prev(func_item_iterator_t *fii, testf_t *testf);\
...
public:
  ...
  bool next(testf_t *func) { return func_item_iterator_next(this, func); }
  bool prev(testf_t *func) { return func_item_iterator_prev(this, func); }
  bool next_addr(void) { return next(NULL); }
  bool next_head(void) { return next(isHead); }
  bool next_code(void) { return next(isCode); }
  bool next_data(void) { return next(isData); }
  bool next_not_tail(void) { return next(isNotTail); }
  bool prev_addr(void) { return prev(NULL); }
  bool prev_head(void) { return prev(isHead); }
  bool prev_code(void) { return prev(isCode); }
  bool prev_data(void) { return prev(isData); }
  bool prev_not_tail(void) { return prev(isNotTail); }
...
bool set_noret_for_exit(ea_t ea, const char *func_name=NULL);

...
idaman char *ida_export calcexpr_long(ea_t where, const char *line, sval_t *res);
inline char *idaapi calcexpr_long(ea_t where, const char *line, uval_t *res)
        { return calcexpr_long(where, line, (sval_t *)res); }
...
idaman char *ida_export calcexpr(ea_t where, const char *line, value_t *rv);
...
idaman char *ida_export CompileEx(const char *file, bool del_macros);

inline char *ida_export Compile(const char *file)
  { return CompileEx(file, true); }
...
idaman char *ida_export CompileLine(const char *line, uval_t (idaapi*_getname)(const char *name)=NULL);
...
idaman char *ida_export ExecuteLine(
                const char *line,
                const char *func,
...
idaman char *ida_export ExecuteFile(
                const char *file,
                const char *func,
                int argsnum,
                const value_t args[],

...
idaman bool ida_export do_name_anyway(ea_t ea,const char *name,int maxlen=0); // give a name: try 100 times with different suffixes (_##)
...
idaman char *ida_export make_visible_name(char *dstname,const char *srcname);
...
// returns: NULL - if address is not valid, no segment or other failure
//          otherwise - ptr to name in a static storage
//                 you should copy it to another place before
//                 working with other name-related functions

idaman char *ida_export get_name_expr(ea_t from,
                                      int n,
                                      ea_t ea,
                                      adiff_t off,
                                      int flags=GETN_APPZERO);
                                // get name expression at 'ea' for item 'from'
                                // the value of name expression should
                                // be equal to 'off'
                                // off == BADADDR - don't check
...
idaman int ida_export get_nice_colored_name(ea_t ea,
                                            char *buf,
                                            size_t bufsize,
                                            int flags=0);

...
inline size_t   get_enum_qty(void)      { return enums.altval(ENUM_QTY_IDX); }
...
// get name of enum
inline char*    get_enum_name(enum_t id)                { return id > MAXADDR ? netnode(id).name() : NULL;  }

// get enum comment
inline char*    get_enum_cmt(enum_t id,bool repeatable) { return id > MAXADDR ? netnode(id).supval(repeatable != 0) : NULL; }

// get the number of the members of the enum
inline size_t   get_enum_size(enum_t id)                { return id > MAXADDR ? netnode(id).altval(ENUM_QTY_IDX) : 0; }

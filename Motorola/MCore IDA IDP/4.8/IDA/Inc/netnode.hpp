...
idaman char *ida_export netnode_name            (nodeidx_t num);
idaman bool  ida_export netnode_rename          (nodeidx_t num, const char *newname,size_t namlen);
idaman char *ida_export netnode_value           (nodeidx_t num);
...
idaman char *ida_export netnode_hashval         (nodeidx_t num, const char *idx,char tag);
...
idaman char *ida_export netnode_hashnxt         (nodeidx_t num, const char *idx,char tag);
idaman char *ida_export netnode_hashlast        (nodeidx_t num, char tag);
idaman char *ida_export netnode_hashprev        (nodeidx_t num, const char *idx,char tag);
...
idaman size_t ida_export netnode_last_length    (void);
...
// Value of netnode as a long number:
  ...
  bool value_exists(void)    { return value() != NULL; }
  nodeidx_t long_value(void) { char *p = value(); return (p==NULL)?BADNODE:*(nodeidx_t*)p; }
...
// returns: NULL - element doesn't exist
  ...
  char *supval(sval_t alt, char tag=stag) const
...
// Returns the length of the last item returned by supval/hashval/hashnext
  static size_t last_length(void)
...
// returns: 1 - ok, 0 - some error
  ...
  bool hashdel_all(char tag=htag) { return supdel_all(tag); }
...
//
  ...
  static bool createbase(const char *fname,nodeidx_t iniNumber=0); // Create base
...
//  bool check(nodeidx_t name)                 // Check and access node type

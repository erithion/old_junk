...
// 53   ??.09.00        API is changed
// 54   16.10.00        struct offset delta is added
// 55   28.01.01        selr_t is 32 bit
// 56   15.05.01        microcode interface is changed
// 57   31.08.01        microcode is removed from the kernel
// 58   07.12.01        interface is changed
// 59   19.12.01        cmd has 6 operands
// 60   05.03.02        more idc functions; low/high asm dirs
// 61   06.08.02	interface is changed
// 62   19.11.02	debugger memory functions are added
// 63   31.11.02	move_segm() added
// 64   03.01.03	netnode.altshift() added
// 65   03.01.03	loaders are aware of the move_segm() function
// 66   28.07.03	API is changed
// 67   01.12.03	colors are added
// 68   25.03.04	apply_type_to_stkarg() is added
// 69   18.05.04	getsysfile() interface is changed
// 70   24.08.04	get_user_idadir() is added
// 71   21.09.04	ph.setsgr has additional parameter; debugger interface has been changed
// 72   21.10.04	many changes
// 73   26.10.04	frame pointer delta has been added
// 74   21.02.05	API is modified to be more secure
// 75   25.02.05	added qisdir()
...
//
  const char *(idaapi* get_type_name)(flags_t flag,ea_t id);
...
#define CUSTOM_CMD_ITYPE 0x8000
        ...
        custom_outop,           // args: op_t *op
                                ...
                                //   the 'u_line' variable points to the output buffer
                                // returns: 2
...
//
  ...
  int   (idaapi* is_far_jump)(int icode);      // is indirect far jump or call instruction?
...
//-----------------------------------------------------------------------
// Micro instruction support for the modules
// These functions are simple thunks.
// They pass execution to the high level module.
// These functions are the only functions allowed to use by the modules
// to generate microinstructions. The details are not settled down yet.

class bitset_t;
class mop_t;
class mblock_t;
typedef uchar internal_mcode_t;

idaman void ida_export bitset_t_kill   (bitset_t *);
idaman void ida_export bitset_t_add    (bitset_t *, int bit, int width);
idaman bool ida_export bitset_t_is_empty(const bitset_t *);
idaman void ida_export mop_t_erase     (mop_t *);
idaman void ida_export mop_t_copy_immediate_number(mop_t *, ea_t ea, const op_t &x, ulong value);
idaman int  ida_export mblock_t_emit_l (mblock_t *, internal_mcode_t code,int width,ulong l,ulong r,ulong d);
idaman int  ida_export mblock_t_emit_o (mblock_t *, internal_mcode_t code,int width,const mop_t *l,const mop_t *r,const mop_t *d);

// This function is called from the high level module to initialize the thunks:

idaman void ida_export init_micro_thunks(
  void idaapi bitset_t_kill   (bitset_t *),
  void idaapi bitset_t_add    (bitset_t *, int bit, int width),
  bool idaapi bitset_t_is_empty(const bitset_t *),
  void idaapi mop_t_erase     (mop_t *),
  void idaapi mop_t_copy_immediate_number(mop_t *,ea_t, const op_t &, ulong),
  int  idaapi mblock_t_emit_l (mblock_t *, internal_mcode_t code,int width,ulong l,ulong r,ulong d),
  int  idaapi mblock_t_emit_o (mblock_t *, internal_mcode_t code,int width,const mop_t *l,const mop_t *r,const mop_t *d)
                                        );
#pragma pack(pop)
#endif // _IDP_HPP

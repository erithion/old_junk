...
//      IDA requires an address to belong to one segment.
...
//      All addresses within a segment should be enabled, i.e. each address
//      in a segment has its flag value (see bytes.hpp)
//      Each segment has its start address, ending address and represents
//      a contiguous range of addresses. However, IDA allows holes between
//      segments.
//
...
//      programs there is special array for translation of selectors to
//      segment base paragraphs. The selector is 32/64 bit value.
//
//      Segment base paragraph determines offsets in the segment.
//      If start address of segment == (base << 4) then the first
//      offset in the segment will be 0. Of course, start address should be
//      higher or equal to (base << 4).
//      We will call offsets in the segment 'virtual addresses'.
...
#define SFL_LOADER      0x10
  ...
  void set_loader_segm(bool debseg) { setflag(flags, SFL_LOADER, debseg); }
...
{
  sel_t n;
  ea_t base;    // in fact this is an address >> 4
};

// Get description of selector (0..get_selector_qty()-1)

idaman selector_t ida_export getn_selector(int n);
...
#define MAX_SELECTORS   4096            // size of selector translation table
...
//      permanent - 0: don't disable addresses belonging to the segment
//                     if you want to create another segment, you need
//                     this setting
//                  1: delete all information about the segment and
//                     disable addresses
// returns 1-ok,0-failed, no segment at 'ea'.

idaman int ida_export del_segm(ea_t ea,int permanent);
...
// returns: NULL or segment comment (static storage!)

inline char   *get_segment_comment(const segment_t *s,int repeatable)
                { return segs.get_area_comment(s,repeatable); }
...
//
inline void    set_segment_comment(segment_t *s,const char *cmt,int repeatable)
                { segs.set_area_comment(s,cmt,repeatable); }
...
inline void    del_segment_comment(segment_t *s,int repeatable)
                { segs.del_area_comment(s,repeatable); }
...
//         otherwise returns segment name

idaman char *ida_export get_true_segm_name(const segment_t *s);           // get segment name


// Get segment name by pointer to segment
//      s - pointer to segment
// returns NULL if s == NULL
//         otherwise returns segment name displayable on the screen

idaman char *ida_export get_segm_name(const segment_t *s);                // get segment name


// Get segment name, in: any address within the segment
//      ea - any linear address within the segment
// returns NULL if no segment exists
//         otherwise returns segment name

inline char *get_segm_name(ea_t ea) { return get_segm_name(getseg(ea)); }
...
//      s - pointer to segment
// returns NULL if s == NULL
//         otherwise returns segment class (at least empty string)

idaman char *ida_export get_segm_class(const segment_t *s);       // get segment class
...
char *get_based_segm_expr(ea_t from, sel_t basesel, sel_t sel, char *buf, size_t bufsize);

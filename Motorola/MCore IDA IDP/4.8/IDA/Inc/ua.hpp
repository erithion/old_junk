...
idaman int ida_export get_operand_immvals(ea_t ea, int n, uval_t *v);
...
#define   OOFW_32       0x0030      //   32 bit width
#define   OOFW_64       0x0040      //   32 bit width
...
inline void idaapi OutLong(uval_t Word, char radix)
{
  OutLine(btoa(Word, radix));
}
...
idaman int ida_export ua_stkvar(const op_t &x, adiff_t v);
...
//--------------------------------------------------------------------------
idaman int ida_export ua_code(ea_t ea);  // Convert to code, top level function

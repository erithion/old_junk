...
void idaapi HelpTerm(void);

idaman int   ida_export ivalue_size( void );      /* Return size of ivalue[123] buf       */
idaman char *ida_export ivalue1(help_t mes);      /* Return answer in static buffer       */
idaman char *ida_export ivalue2(help_t mes);      /* if mes == -1, then return buffer     */
idaman char *ida_export ivalue3(help_t mes);
idaman char *ida_export ivalue(help_t mes);       /* Return answer in dynamic memory      */
                                /* if mes == -1, then return NULL       */
char *GetMessage(help_t para,help_t mes);
                                /* For C-scape, returns answer          */
                                /* in buffer 1                          */

help_t HelpPutMessage(char *msg); /* add a dynamic message              */
...
class itext
{
  char *ptr;
public:
  itext(help_t mes)     { ptr = ivalue(mes); }
  ~itext(void)          { delete ptr; }
  operator char*()      { return ptr; }
};

void    showhelp(help_t);       // Show help message in the separate window
                                // and allow user to escape to help system.

...
//
//        Each byte of a disassembled program is stored 32-bit value called
//        "flags" which also stores addtional information about the byte. The
//        structure of "flags" is presented below. You are not allowed to
//        inspect or modify the individual bits of flags directly. Use the
//        special functions provided to inspect and/or modify flags.
//--------------------------------------------------------------------------
...
idaman int ida_export FlagsEnable(ea_t startEA,ea_t endEA);
...
idaman int ida_export FlagsDisable(ea_t startEA,ea_t endEA);
...
idaman ea_t ida_export nextthat(ea_t ea,ea_t maxea,bool (idaapi*testf)(flags_t flags));
...
idaman ea_t ida_export prevthat(ea_t ea,ea_t minea,bool (idaapi*testf)(flags_t flags));
...
idaman flags_t ida_export getFlags(ea_t ea);
...
// Get byte value from flags (only 8 bits from the database)
// Use get_byte() instead

inline uchar idaapi _byteValue(flags_t F) { return ((uchar)(F & MS_VAL)); }


// Delete byte value from flags. The corresponding byte becomes
// uninitialized.

inline void idaapi delValue(ea_t ea) { clrFlbits(ea,FF_IVL); }


// Does the specified address have a byte value (is initialized?)

inline bool idaapi isLoaded(ea_t ea) { return hasValue(getFlags(ea)); }
...
idaman void ida_export put_byte(ea_t ea,ulong x);
...
idaman void ida_export patch_byte(ea_t ea,ulong x);
...
idaman char *ida_export get_forced_operand(ea_t ea,int n);
...
idaman bool ida_export doData(ea_t ea,ulong size,tid_t tid);
...
idaman bool ida_export do_data_ex(ea_t ea,flags_t dataflag,ulong size,tid_t tid);
...
idaman bool ida_export doByte(ea_t ea,ulong size);             // 1-byte
idaman bool ida_export doWord(ea_t ea,ulong size);             // 2-byte
idaman bool ida_export doDwrd(ea_t ea,ulong size);             // 4-byte
idaman bool ida_export doQwrd(ea_t ea,ulong size);             // 8-byte
idaman bool ida_export doOwrd(ea_t ea,ulong size);             // 16-byte
idaman bool ida_export doTbyt(ea_t ea,ulong size);             // ?-byte (defined in IDP)
idaman bool ida_export doFloat(ea_t ea,ulong size);            // 4-byte
idaman bool ida_export doDouble(ea_t ea,ulong size);           // 8-byte
idaman bool ida_export doPackReal(ea_t ea,ulong size); // ?-byte (defined in IDP)
idaman bool ida_export doASCI(ea_t ea,ulong size);              // don't use this! use make_ascii_string()
idaman bool ida_export doStruct(ea_t ea,ulong size,tid_t tid);
idaman bool ida_export doAlign(ea_t ea,ulong length,int alignment);
                                                // alignment: 0 or 2..32
                                                // if it is 0, is will be calculated
idaman int  ida_export calc_min_align(ulong length);    // returns: 1..32
...
idaman bool ida_export do16bit(ea_t ea,ulong length);  // convert to 16-bit quanztity
...
idaman size_t ida_export get_max_ascii_length(ea_t ea, long strtype, bool ignore_heads=false);
...
idaman bool ida_export get_ascii_contents(ea_t ea, size_t len, long type, char *buf, size_t bufsize);
...
idaman char *ida_export print_ascii_string_type(char *buf, size_t bufsize, long strtype);
...
idaman typeinfo_t *ida_export get_typeinfo(ea_t ea, int n, flags_t flags, typeinfo_t *buf);
...
idaman bool ida_export set_typeinfo(ea_t ea, int n, flags_t flag, const typeinfo_t *ti);
...
idaman char *ida_export get_cmt(ea_t ea,bool rptble);
...
idaman char *ida_export get_rpt_cmt(ea_t ea);
...
idaman char *ida_export get_any_cmt(ea_t ea,color_t *cmttype);
...
idaman bool ida_export equal_bytes(ea_t ea,const uchar *image,const uchar *mask,
